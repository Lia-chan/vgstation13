...
/obj/structure/freezedmob/proc/icecheck()
	if (src.ice <= 0)
		...
		if(!src.occupant) // I wonder how, but it happens - Nanodesu
			del(src)
			world.log << "DEBUG: Deleted freezed mob because no occupant! [src]"
			return
		src.occupant.loc = src.loc
		...
		if(M.stat != 2)
			if (!M.client)
				for(var/mob/dead/observer/ghost in world)
					if(ghost.client && ghost.mind && ghost.mind.current && ghost.mind.current == M)
						ghost.cancel_camera()
